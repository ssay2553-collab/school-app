...
export default function ManageFees() {
  ...
  const [individualBillOverrides, setIndividualBillOverrides] = useState<Record<string, string>>({});
  ...
  const [selectorModal, setSelectorModal] = useState<{ visible: boolean; type: "class" | "year" | "term" | null }>({ visible: false, type: null });
  ...
  const fetchStudents = useCallback(async (isFirstLoad = false) => {
    if (!selectedClassId || isFetchingRef.current || !academicYear || !term) return;
    ...
    try {
      let q = query(collection(db, "users"), where("role", "==", "student"), where("classId", "==", selectedClassId), orderBy("profile.firstName"), limit(PAGE_SIZE));
      if (!isFirstLoad && lastVisibleRef.current) q = query(q, startAfter(lastVisibleRef.current));
      ...
      const batch: StudentDraft[] = studentDocs.map(d => {
        ...
        return {
          uid: d.id,
          fullName: `${userData.profile?.firstName || ""} ${userData.profile?.lastName || ""}`.trim() || "Student",
  ...
  const saveFees = async () => {
    if (!canEdit) return Alert.alert("Denied", "You don't have permission to modify billing.");
    setSaving(true);
    try {
  ...
  const handleLogPayment = async () => {
    if (!canEdit) return Alert.alert("Denied", "You don't have permission to record payments.");
    const amount = parseFloat(paymentAmount);
    if (isNaN(amount) || !selectedStudent || !receivedFrom.trim()) return Alert.alert("Error", "Incomplete data");
    setSaving(true);
    try {
      ...
      batch.update(doc(db, "studentFeeRecords", recordId), { amountPaid: increment(amount), balance: increment(-amount), payments: arrayUnion(entry) });
      batch.update(doc(db, "users", selectedStudent.uid), { walletBalance: increment(-amount) });
      await batch.commit();
      setPaymentAmount(""); setReceivedFrom(""); setPaymentModalVisible(false);
      fetchStudents(true);
      Alert.alert("Success", `Payment recorded. Receipt: ${serial}`);
    } catch { Alert.alert("Error", "Payment failed"); } finally { setSaving(false); }
  };
    ...
    Alert.alert("Confirm Deletion", "Are you sure you want to delete this transaction? This will automatically adjust the student's balance.", [
      ...
      { text: "Delete Permanently", style: "destructive", onPress: async () => {
        ...
        try {
          ...
          Alert.alert("Success", "Transaction reverted and balance updated.");
        } catch { Alert.alert("Error", "Failed to delete"); } finally { setSaving(false); }
      }},
    ]);
  };
  ...
  const renderStudentItem = ({ item }: { item: StudentDraft }) => {
    ...
    return (
      <Animatable.View animation="fadeInUp" duration={400} style={styles.cardWrapper}>
        ...
        >
          <View style={[styles.statusIndicator, { backgroundColor: hasDebt ? VIBE.danger : VIBE.success }]} />
          <View style={styles.cardContent}>
            ...
            <View style={styles.rightSection}>
              {activeMode === "billing" ? (
                <View style={[styles.billingBubble, hasActiveBill && styles.billingBubbleActive, hasOverride && styles.billingBubbleOverride]}>
                  <Text style={[styles.bubbleSym, (hasActiveBill || hasOverride) && styles.textWhite]}>₵</Text>
                  <TextInput
                    style={[styles.bubbleInput, (hasActiveBill || hasOverride) && styles.textWhite]}
                    placeholder="0"
                    placeholderTextColor={hasActiveBill ? "rgba(255,255,255,0.6)" : VIBE.muted}
                    value={String(currentBillValue)}
                    onChangeText={v => setIndividualBillOverrides(p => ({ ...p, [item.uid]: v }))}
                ...
                <View style={styles.actionIcons}>
                   <TouchableOpacity onPress={(e) => { e.stopPropagation(); router.push({ pathname: "/admin-dashboard/student-fee-history", params: { studentId: item.uid, academicYear, term } }); }} style={styles.historyCircle}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <LinearGradient colors={[SCHOOL_CONFIG.brandPrimary, SCHOOL_CONFIG.brandSecondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.headerTop}>
           <View style={styles.navBar}>
              ...
              <TouchableOpacity onPress={exportPDF} style={styles.headerIconBtn}><SVGIcon name="share-outline" size={24} color="#fff" /></TouchableOpacity>
           </View>
           <View style={styles.selectorGrid}>
              <TouchableOpacity style={styles.glassPill} onPress={() => setSelectorModal({ visible: true, type: "class" })}>
                 <Text style={styles.glassLabel}>TARGET CLASS</Text>
                 <Text style={styles.glassValue} numberOfLines={1}>{selectedClassId === "all" ? "All Classes" : classes.find(c => c.id === selectedClassId)?.name || "Select Class"}</Text>
              </TouchableOpacity>
              <View style={styles.glassPill}><Text style={styles.glassLabel}>ACADEMIC YEAR</Text><Text style={styles.glassValue}>{academicYear || "Not Set"}</Text></View>
              <View style={styles.glassPill}><Text style={styles.glassLabel}>TERM</Text><Text style={styles.glassValue}>{term || "Not Set"}</Text></View>
           </View>
        </LinearGradient>
          ...
          <>
            ...
            <View style={styles.modeToggleArea}>
              <View style={styles.modeTabs}>
                  <TouchableOpacity style={[styles.modeTab, activeMode === "payment" && styles.activeModeTab]} onPress={() => setActiveMode("payment")}>
                    <SVGIcon name="cash" size={18} color={activeMode === "payment" ? "#fff" : VIBE.muted} />
                    <Text style={[styles.modeTabText, activeMode === "payment" && { color: '#fff' }]}>PAYMENTS</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modeTab, activeMode === "billing" && styles.activeModeTab]} onPress={() => setActiveMode("billing")}>
                    <SVGIcon name="document-text" size={18} color={activeMode === "billing" ? "#fff" : VIBE.muted} />
                    <Text style={[styles.modeTabText, activeMode === "billing" && { color: '#fff' }]}>BILLING</Text>
                  </TouchableOpacity>
              </View>
            </View>
          </>
      ...
      {isConfigMissing ? (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity style={[styles.linkBtn, { backgroundColor: VIBE.primary }]} onPress={() => router.push("/academic-calendar")}><Text style={styles.linkBtnText}>Go to Calendar</Text></TouchableOpacity>
        </View>
      ) : (
        <View style={styles.mainBody}>
          {activeMode === "billing" ? (
            <View style={styles.bulkActionStrip}>
              <View style={styles.bulkInputContainer}>
                  ...
                  <TextInput placeholder="Set Bulk Amount" placeholderTextColor={VIBE.muted} style={styles.bulkInput} keyboardType="numeric" value={termBillAmount} onChangeText={setTermBillAmount} editable={canEdit} />
              </View>
              <TouchableOpacity style={styles.checkAllBtn} onPress={() => { const allSelected = filteredStudents.length > 0 && filteredStudents.every(s => selectedStudentUids.has(s.uid)); setSelectedStudentUids(new Set(allSelected ? [] : filteredStudents.map(s => s.uid))); }}>
                  <SVGIcon name={filteredStudents.length > 0 && filteredStudents.every(s => selectedStudentUids.has(s.uid)) ? "checkbox" : "square-outline"} size={28} color={VIBE.primary} />
                  <Text style={styles.checkAllText}>SELECT ALL</Text>
              </TouchableOpacity>
            </View>
          ) : (
            students.length > 0 && !searchQuery && (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.statsDashboard}>
                <Animatable.View animation="zoomIn" style={[styles.statBox, { backgroundColor: VIBE.info }]}>
                    <Text style={styles.statLabel}>EXPECTED</Text>
                    <Text style={styles.statValue}>₵{stats.expected.toLocaleString()}</Text>
                    <View style={styles.statIcon}><SVGIcon name="analytics" size={24} color="rgba(255,255,255,0.3)" /></View>
                </Animatable.View>
                <Animatable.View animation="zoomIn" delay={100} style={[styles.statBox, { backgroundColor: VIBE.success }]}>
                    <Text style={styles.statLabel}>RECEIVED</Text>
                    <Text style={styles.statValue}>₵{stats.received.toLocaleString()}</Text>
                    <View style={styles.statIcon}><SVGIcon name="wallet" size={24} color="rgba(255,255,255,0.3)" /></View>
                </Animatable.View>
                <Animatable.View animation="zoomIn" delay={200} style={[styles.statBox, { backgroundColor: VIBE.danger }]}>
                    <Text style={styles.statLabel}>OUTSTANDING</Text>
                    <Text style={styles.statValue}>₵{stats.balance.toLocaleString()}</Text>
                    <View style={styles.statIcon}><SVGIcon name="alert-circle" size={24} color="rgba(255,255,255,0.3)" /></View>
                </Animatable.View>
              </ScrollView>
            )
          )}
          ...
          <View style={styles.listHeaderRow}>
            ...
            <View style={styles.filterChips}>
                {["all", "debt", "cleared"].map(f => (
                  <TouchableOpacity 
                    key={f} 
                    style={[styles.filterChip, statusFilter === f && { backgroundColor: VIBE.primary, borderColor: VIBE.primary }]}
                    onPress={() => setStatusFilter(f as any)}
                  >
                    <Text style={[styles.filterChipText, statusFilter === f && { color: '#fff' }]}>
          ...
          <FlatList
            ...
            extraData={{ activeMode, termBillAmount, individualBillOverrides, selectedStudentUids }}
      ...
      {activeMode === "billing" && selectedStudentUids.size > 0 && canEdit && (
        <Animatable.View animation="bounceIn" style={styles.fabWrap}>
           <TouchableOpacity style={styles.mainFab} onPress={() => setBillModalVisible(true)}>
              <LinearGradient colors={[VIBE.primary, VIBE.purple]} style={styles.fabGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                 <Text style={styles.fabText}>APPLY BILLS ({selectedStudentUids.size})</Text>
      ...
      <Modal visible={selectorModal.visible} transparent animationType="slide">
        <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={() => setSelectorModal({ visible: false, type: null })}>
           <View style={styles.sheetBody}>
              <View style={styles.sheetHandle} />
              <Text style={styles.sheetTitle}>Select Class</Text>
              <ScrollView>
                 <TouchableOpacity style={[styles.sheetItem, selectedClassId === "all" && styles.activeSheetItem]} onPress={() => { setSelectedClassId("all"); setSelectorModal({ visible: false, type: null }); }}>
                    <Text style={[styles.sheetItemText, selectedClassId === "all" && styles.activeSheetItemText]}>All Classes</Text>
                 </TouchableOpacity>
                 {classes.map(c => <TouchableOpacity key={c.id} style={[styles.sheetItem, selectedClassId === c.id && styles.activeSheetItem]} onPress={() => { setSelectedClassId(c.id); setSelectorModal({ visible: false, type: null }); }}><Text style={[styles.sheetItemText, selectedClassId === c.id && styles.activeSheetItemText]}>{c.name}</Text></TouchableOpacity>)}
              </ScrollView>
           </View>
        </TouchableOpacity>
      </Modal>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.overlay}>
           <View style={styles.paymentModal}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
                 ...
                 <TouchableOpacity style={[styles.saveBtn, { backgroundColor: VIBE.primary }]} onPress={handleLogPayment} disabled={saving || !canEdit}>
                    {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>CONFIRM PAYMENT</Text>}
                 </TouchableOpacity>

                 <View style={styles.historyBlock}>
                    <View style={styles.historyHeader}>
                       <Text style={styles.blockTitle}>Term Transactions</Text>
                       <TouchableOpacity onPress={() => { if (selectedStudent) { setPaymentModalVisible(false); router.push({ pathname: "/admin-dashboard/student-fee-history", params: { studentId: selectedStudent.uid, academicYear, term } }); } }}>
                          <Text style={[styles.viewFullText, { color: VIBE.primary }]}>Full Ledger</Text>
                       </TouchableOpacity>
                    </View>
                    {selectedStudent?.payments?.length ? selectedStudent.payments.slice().reverse().map((p, i) => (
      ...
      <Modal visible={billModalVisible} transparent animationType="fade">
         <View style={styles.overlayCenter}>
            <View style={styles.alertCard}>
               ...
               <Text style={styles.alertText}>Apply rates to {selectedStudentUids.size} accounts?</Text>
               <View style={styles.alertBtnRow}>
                  <TouchableOpacity onPress={() => setBillModalVisible(false)} style={styles.alertBtnSec}><Text style={styles.alertBtnTextSec}>Cancel</Text></TouchableOpacity>
                  <TouchableOpacity onPress={saveFees} style={[styles.alertBtnPri, { backgroundColor: VIBE.primary }]}><Text style={styles.alertBtnTextPri}>Confirm</Text></TouchableOpacity>
               </View>
            </View>
         </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#fff', borderBottomLeftRadius: 30, borderBottomRightRadius: 30, ...SHADOWS.medium },
  headerTop: { paddingHorizontal: 20, paddingTop: Platform.OS === 'ios' ? 10 : 40, paddingBottom: 30, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
  navBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 25 },
  ...
  glassPill: { flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 15, padding: 10 },
  glassLabel: { fontSize: 8, fontWeight: '900', color: 'rgba(255,255,255,0.6)', marginBottom: 2 },
  glassValue: { fontSize: 13, fontWeight: '800', color: '#fff' },
  searchStrip: { flexDirection: 'row', paddingHorizontal: 20, marginTop: -25, alignItems: 'center', gap: 10, marginBottom: 20 },
  searchBar: { flex: 1, height: 50, backgroundColor: '#fff', borderRadius: 25, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, ...SHADOWS.medium },
  searchInput: { flex: 1, marginLeft: 10, fontSize: 14, fontWeight: '600', color: VIBE.text },
  refreshRound: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', ...SHADOWS.small },
  modeToggleArea: { paddingHorizontal: 20, marginBottom: 20 },
  modeTabs: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 15, padding: 5, ...SHADOWS.small },
  modeTab: { flex: 1, height: 45, borderRadius: 12, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8 },
  ...
  statBox: { width: 180, height: 110, borderRadius: 30, padding: 20, justifyContent: 'center' },
  statLabel: { fontSize: 9, fontWeight: '900', color: 'rgba(255,255,255,0.7)', letterSpacing: 1 },
  statValue: { fontSize: 20, fontWeight: '900', color: '#fff', marginTop: 5 },
  statIcon: { position: 'absolute', bottom: 15, right: 15 },
  mainBody: { flex: 1 },
  bulkActionStrip: { flexDirection: 'row', paddingHorizontal: 20, paddingBottom: 20, alignItems: 'center', gap: 15 },
  bulkInputContainer: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', height: 56, borderRadius: 20, paddingHorizontal: 20, ...SHADOWS.small },
  bulkSym: { fontSize: 18, fontWeight: '900', color: VIBE.primary, marginRight: 10 },
  ...
  listHeaderRow: { flexDirection: 'row', paddingHorizontal: 20, justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  listTitle: { fontSize: 12, fontWeight: '900', color: VIBE.muted, letterSpacing: 1, textTransform: 'uppercase' },
  filterChips: { flexDirection: 'row', gap: 8 },
  filterChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 15, backgroundColor: '#fff', borderWidth: 1, borderColor: VIBE.border },
  filterChipText: { fontSize: 10, fontWeight: '900', color: VIBE.muted },
  flatListContent: { paddingHorizontal: 20, paddingBottom: 120 },
  cardWrapper: { marginBottom: 15 },
  financeCard: { backgroundColor: '#fff', borderRadius: 25, padding: 18, ...SHADOWS.small, borderWidth: 1, borderColor: VIBE.border },
  selectedCard: { borderColor: VIBE.primary, backgroundColor: VIBE.primary + '05' },
  statusIndicator: { position: 'absolute', left: 0, top: 0, bottom: 0, width: 6, borderTopLeftRadius: 25, borderBottomLeftRadius: 25 },
  cardContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  leftSection: { flex: 1, flexDirection: 'row', alignItems: 'center', marginRight: 15, minWidth: 0 },
  avatar: { width: 48, height: 48, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
  ...
  rightSection: { width: 120, alignItems: 'flex-end', justifyContent: 'center' },
  billingBubble: { 
  ...
  billingBubbleActive: { backgroundColor: VIBE.primary, borderColor: VIBE.primary },
  billingBubbleOverride: { backgroundColor: VIBE.purple, borderColor: VIBE.purple },
  bubbleSym: { fontSize: 14, fontWeight: '900', color: VIBE.muted, marginRight: 4 },
  bubbleInput: { fontSize: 14, fontWeight: 'bold', color: VIBE.text, textAlign: 'center', minWidth: 40, padding: 0 },
  actionIcons: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  historyCircle: { width: 38, height: 38, borderRadius: 19, justifyContent: 'center', alignItems: 'center', backgroundColor: VIBE.bg },
  fabWrap: { position: 'absolute', bottom: 30, left: 20, right: 20 },
  mainFab: { height: 64, borderRadius: 24, overflow: 'hidden', ...SHADOWS.large },
  fabGrad: { flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 12 },
  fabText: { color: '#fff', fontSize: 16, fontWeight: '900', letterSpacing: 0.5 },
  overlay: { flex: 1, backgroundColor: 'rgba(15,23,42,0.6)', justifyContent: 'flex-end' },
  overlayCenter: { flex: 1, backgroundColor: 'rgba(15,23,42,0.8)', justifyContent: 'center', padding: 25 },
  sheetBody: { backgroundColor: '#fff', borderTopLeftRadius: 40, borderTopRightRadius: 40, padding: 25, maxHeight: '80%' },
  sheetHandle: { width: 40, height: 5, backgroundColor: VIBE.border, borderRadius: 3, alignSelf: 'center', marginBottom: 20 },
  sheetTitle: { fontSize: 18, fontWeight: '900', color: VIBE.text, textAlign: 'center', marginBottom: 25 },
  sheetItem: { padding: 20, borderRadius: 18, marginBottom: 8, backgroundColor: VIBE.bg },
  activeSheetItem: { backgroundColor: VIBE.primary },
  sheetItemText: { fontSize: 16, fontWeight: '700', color: VIBE.text, textAlign: 'center' },
  activeSheetItemText: { color: '#fff' },
  paymentModal: { backgroundColor: '#fff', borderTopLeftRadius: 45, borderTopRightRadius: 45, padding: 30, maxHeight: '90%' },
  modalTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 35 },
  modalStudentName: { fontSize: 20, fontWeight: '900', color: VIBE.text },
  closeRound: { width: 44, height: 44, borderRadius: 22, backgroundColor: VIBE.bg, justifyContent: 'center', alignItems: 'center' },
  modalInputs: { gap: 15, marginBottom: 30 },
  pillInput: { backgroundColor: VIBE.bg, borderRadius: 24, padding: 18, borderWidth: 1, borderColor: VIBE.border, fontSize: 20, fontWeight: '900', color: VIBE.text },
  methodGrid: { flexDirection: 'row', gap: 10, marginBottom: 30 },
  methodBtn: { flex: 1, height: 48, borderRadius: 15, borderWidth: 1, borderColor: VIBE.border, justifyContent: 'center', alignItems: 'center', backgroundColor: VIBE.bg },
  methodText: { fontSize: 12, fontWeight: '800', color: VIBE.muted },
  saveBtn: { height: 64, borderRadius: 24, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 12, ...SHADOWS.medium },
  saveBtnText: { color: '#fff', fontSize: 16, fontWeight: '900', letterSpacing: 1 },
  historyBlock: { marginTop: 40, borderTopWidth: 1, borderTopColor: VIBE.border, paddingTop: 35 },
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 25 },
  blockTitle: { fontSize: 15, fontWeight: '900', color: VIBE.text, letterSpacing: 0.5 },
  viewFullText: { fontSize: 13, fontWeight: '800' },
  transactionTile: { backgroundColor: VIBE.bg, padding: 20, borderRadius: 24, marginBottom: 12, borderWidth: 1, borderColor: VIBE.border },
  tileHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  tileAmt: { fontSize: 18, fontWeight: '900', color: VIBE.primary },
  tileDetail: { fontSize: 13, color: VIBE.text, fontWeight: '700' },
  tileDate: { fontSize: 11, color: VIBE.muted, fontWeight: '800', marginTop: 8 },
  noHistory: { textAlign: 'center', color: VIBE.muted, marginTop: 20, fontStyle: 'italic' },
  alertCard: { backgroundColor: '#fff', width: '100%', borderRadius: 35, padding: 30, alignItems: 'center' },
  alertTitle: { fontSize: 22, fontWeight: '900', color: VIBE.text, marginBottom: 12 },
  alertText: { fontSize: 14, color: VIBE.muted, textAlign: 'center', lineHeight: 22, marginBottom: 35 },
  alertBtnRow: { flexDirection: 'row', gap: 15 },
  alertBtnPri: { flex: 1.5, height: 60, borderRadius: 20, justifyContent: 'center', alignItems: 'center', ...SHADOWS.medium },
  alertBtnSec: { flex: 1, height: 60, backgroundColor: VIBE.bg, borderRadius: 20, justifyContent: 'center', alignItems: 'center' },
  ...
  emptyTitle: { fontSize: 20, fontWeight: "900", color: "#1E293B", marginTop: 20, marginBottom: 10 },
  linkBtn: { paddingHorizontal: 30, paddingVertical: 15, borderRadius: 20, marginTop: 20, ...SHADOWS.medium },
