...
export default function StaffPayrollScreen() {
  ...
  const canView =
    isSuperAdmin ||
    payrollPermission === "full" ||
    payrollPermission === "view" ||
    payrollPermission === "edit";
  const canEditSalary =
    isSuperAdmin ||
    payrollPermission === "full" ||
    payrollPermission === "edit";
  ...
  useEffect(() => {
    ...
    const loadCache = async () => {
      ...
      if (cached && isMounted.current) {
        try {
          ...
          parsed.forEach(
            (s: Staff) => (salaries[s.id] = (s.salary || 0).toString()),
          );
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        // Map and group by UID to handle users with multiple roles (e.g. Admin & Teacher)
        ...
        snapshot.docs.forEach((d) => {
          ...
          if (userMap.has(d.id)) {
            // If we encounter the same user (shouldn't happen with unique UIDs in users collection,
            // but good for safety if merging logic changes), or if we want to combine labels.
            const existing = userMap.get(d.id)!;
            if (!existing.role.includes(role)) {
              existing.role = `${existing.role} & ${role}` as any;
            }
          } else {
            userMap.set(d.id, {
              id: d.id,
        ...
        const combined = Array.from(userMap.values()).sort((a, b) =>
  ...
  const handlePostToExpenditure = async () => {
    if (!selectedMonth)
      return showToast({ message: "Select a month first.", type: "warning" });
    if (!selectedYear || !selectedTerm)
      return showToast({
        message: "Academic year and term must be set in settings.",
        type: "error",
      });
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        return showToast({
          message:
            "This payroll period has already been recorded in expenditures.",
          type: "info",
        });
      }
      ...
      >
        ...
        <View style={styles.summaryArea}>
          ...
          <Text style={styles.summaryValue}>
            ₵{" "}
            {schoolTotalPayroll.toLocaleString(undefined, {
          ...
          <View style={[styles.pickerBox, { flex: 1.2, marginLeft: 8 }]}>
            ...
            >
              <Picker.Item
                label="Select..."
                value=""
                color={Platform.OS === "web" ? "#000" : "rgba(255,255,255,0.6)"}
              />
              {[
                "Jan",
                "Feb",
          ...
          <Animatable.View animation="fadeInDown" style={styles.postBanner}>
            <View style={{ flex: 1 }}>
              <Text style={styles.bannerSubtitle}>
                Ready to finalize {selectedMonth}?
              </Text>
              <Text style={styles.bannerTitle}>Commit to Expenditures</Text>
            </View>
            <TouchableOpacity
              style={styles.postBtn}
              onPress={handlePostToExpenditure}
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              ...
              <TouchableOpacity
                ...
                onPress={handleAddNonTeaching}
                disabled={addingStaff}
              >
                {addingStaff ? (
                  <ActivityIndicator color="#fff" />
                ) : (
                  <Text style={styles.modalBtnText}>SAVE STAFF RECORD</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  picker: {
    ...
    color: "#fff",
    fontSize: 12,
    marginLeft: -10, // Offset picker internal padding
  },
