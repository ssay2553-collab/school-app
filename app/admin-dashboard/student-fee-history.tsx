...
export default function StudentFeeHistoryScreen() {
  ...
  const [selectedStudentUid, setSelectedStudentUid] = useState((params.studentId as string) || "");
  ...
  useEffect(() => {
    if (!acadConfig.loading) {
      setSelectedYear(params.academicYear as string || acadConfig.academicYear);
  ...
  useEffect(() => {
    ...
    const loadStudents = async () => {
      ...
      try {
        const q = query(collection(db, "users"), where("role", "==", "student"), where("classId", "==", selectedClassId), orderBy("__name__"));
        const snap = await getDocsCacheFirst(q as any);
        const list = snap.docs.map((d) => ({ uid: d.id, name: `${d.data().profile?.firstName || ""} ${d.data().profile?.lastName || ""}`.trim() || "Student" })).sort((a, b) => a.name.localeCompare(b.name));
        setStudents(list);
        if (!selectedStudentUid && list.length > 0) setSelectedStudentUid(list[0].uid);
      } catch (error) { console.error(error); } finally { setFetchingStudents(false); }
    };
    loadStudents();
  }, [selectedClassId]);

  useEffect(() => {
    if (!selectedStudentUid || !selectedYear || !selectedTerm) { setRecord(null); return; }
    ...
    const unsubscribe = onSnapshot(doc(db, "studentFeeRecords", recordId), (snap) => {
  ...
  const filteredPayments = useMemo(() => {
    ...
    return record.payments.filter((p: any) => 
  ...
  const handleDeletePayment = (payment: any) => {
    Alert.alert("Revert Payment", "Are you sure you want to delete this transaction? The student's balance will be adjusted automatically.", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => {
        ...
        try {
          ...
          await batch.commit();
          Alert.alert("Success", "Transaction deleted and balance reverted.");
        } catch (error) {
          console.error(error);
  ...
  const generatePDF = async () => {
      ...
      <html>
        ...
        <body>
          <div class="receipt-paper">
            ...
            <div class="title">FEES PAYMENT RECEIPT</div>
    ...
    } catch { Alert.alert("Error", "Could not generate PDF receipt"); }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={StyleSheet.flatten([styles.navBar, { backgroundColor: primary }])}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backIcon}>
          <SVGIcon name="arrow-back" size={24} color="#fff" />
        </TouchableOpacity>
        <Text style={styles.navTitle}>Fee Ledger</Text>
        <TouchableOpacity onPress={generatePDF} disabled={!record} style={styles.printIcon}>
          <SVGIcon name="download" size={24} color={record ? "#fff" : "rgba(255,255,255,0.3)"} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.pickerRow}>
            <View style={styles.pickerBox}>
              ...
              <Picker 
                ...
                style={StyleSheet.flatten([styles.picker, Platform.OS === 'web' && { color: '#000', backgroundColor: '#fff', height: 40, borderRadius: 10, marginTop: 15 } as any])} 
                dropdownIconColor={Platform.OS === 'web' ? '#000' : '#fff'}
              >
                {classes.map(c => <Picker.Item key={c.id} label={c.name} value={c.id} color="#000" />)}
              </Picker>
            </View>
            <View style={StyleSheet.flatten([styles.pickerBox, { marginLeft: 10 }])}>
              <Text style={styles.pickerLabel}>YEAR {selectedYear === acadConfig.academicYear ? "(CUR)" : ""}</Text>
              <Picker 
                ...
                dropdownIconColor={Platform.OS === 'web' ? '#000' : '#fff'}
              >
                {availableYears.map(y => <Picker.Item key={y} label={y} value={y} color="#000" />)}
              </Picker>
            </View>
          </View>

          <View style={StyleSheet.flatten([styles.pickerRow, { marginTop: 12 }])}>
            <View style={StyleSheet.flatten([styles.pickerBox, { flex: 1.5 }])}>
              ...
              <Picker 
                ...
                dropdownIconColor={Platform.OS === 'web' ? '#000' : '#fff'}
              >
                {fetchingStudents ? <Picker.Item label="Loading..." value="" color="#000" /> : students.map(s => <Picker.Item key={s.uid} label={s.name} value={s.uid} color="#000" />)}
              </Picker>
            </View>
            <View style={StyleSheet.flatten([styles.pickerBox, { flex: 1, marginLeft: 10 }])}>
              <Text style={styles.pickerLabel}>TERM {selectedTerm === acadConfig.currentTerm ? "(CUR)" : ""}</Text>
              <Picker 
        ...
        <View style={styles.mainContent}>
          {fetchingRecord || deleting ? (
            ...
            <Animatable.View animation="fadeInUp" duration={600} style={styles.receiptPaper}>
              {/* Receipt Header */}
              <View style={styles.paperHeader}>
                ...
                <View style={styles.paperSchoolInfo}>
                  ...
                  <Text style={styles.paperContact}>{SCHOOL_CONFIG.address}</Text>
                </View>
              </View>

              <View style={styles.divider} />
              <Text style={styles.receiptTitle}>FINANCIAL HISTORY</Text>

              {/* Student Info */}
              <View style={styles.paperInfoGrid}>
                <View style={styles.infoItem}><Text style={styles.infoLabel}>STUDENT:</Text><Text style={styles.infoValue}>{record.studentName}</Text></View>
                <View style={styles.infoItem}><Text style={styles.infoLabel}>CLASS:</Text><Text style={styles.infoValue}>{record.className || "N/A"}</Text></View>
                <View style={styles.infoItem}><Text style={styles.infoLabel}>PERIOD:</Text><Text style={styles.infoValue}>{record.term} • {record.academicYear}</Text></View>
                <View style={styles.infoItem}><Text style={styles.infoLabel}>DATE:</Text><Text style={styles.infoValue}>{moment().format("DD/MM/YYYY")}</Text></View>
              </View>
              ...
              <View style={styles.tableContainer}>
                <View style={styles.tableHeader}>
                  ...
                  <Text style={[styles.th, { flex: 1.5 }]}>PAYEE / ADMIN</Text>
                  <Text style={[styles.th, { flex: 1, textAlign: 'right' }]}>AMOUNT</Text>
                  <Text style={[styles.th, { width: 40, textAlign: 'center' }]}>DEL</Text>
                </View>
                {filteredPayments.map((p: any, i: number) => (
                  <View key={i} style={styles.tableRow}>
                    <View style={{ flex: 1.2 }}>
                        <Text style={styles.td}>{moment(p.createdAt).format("DD/MM/YY")}</Text>
                        <Text style={styles.tdRef}>{p.receiptNo || 'RC-' + String(p.createdAt).slice(-6)}</Text>
                    </View>
                    <View style={{ flex: 1.5 }}>
                        <Text style={styles.tdPayee}>{p.receivedFrom || "Self"}</Text>
                        <Text style={styles.tdAdmin}>By: {p.updatedBy || "Admin"}</Text>
                    </View>
                    <Text style={[styles.td, { flex: 1, textAlign: 'right', fontWeight: '900' }]}>₵{Number(p.amount).toFixed(2)}</Text>
              ...
              <View style={styles.totalsSection}>
                <View style={styles.totalsRow}><Text style={styles.totalsLabel}>ARREARS B/F:</Text><Text style={styles.totalsValue}>₵ {record.arrears?.toFixed(2)}</Text></View>
                <View style={styles.totalsRow}><Text style={styles.totalsLabel}>TERM BILL:</Text><Text style={styles.totalsValue}>₵ {record.termBill?.toFixed(2)}</Text></View>
                <View style={styles.totalsRow}><Text style={styles.totalsLabel}>TOTAL PAID:</Text><Text style={[styles.totalsValue, { color: '#10B981' }]}>- ₵ {record.amountPaid?.toFixed(2)}</Text></View>
                <View style={styles.grandTotalRow}><Text style={styles.grandTotalLabel}>BALANCE DUE:</Text><Text style={styles.grandTotalValue}>₵ {record.balance?.toFixed(2)}</Text></View>
              </View>

              <View style={styles.paperFooter}>
                <Text style={styles.footerText}>Computer generated. Reverting transactions updates balances instantly.</Text>
                <Text style={styles.copyrightText}>© {moment().year()} {SCHOOL_CONFIG.fullName}</Text>
              </View>

              <View style={styles.watermark}>
                <SVGIcon name="checkmark-done-circle" size={120} color="rgba(0,0,0,0.02)" />
              </View>
            </Animatable.View>
          ) : selectedStudentUid && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySub}>No financial data for the selected period.</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  filterCard: { padding: 20, borderBottomLeftRadius: 30, borderBottomRightRadius: 30, ...SHADOWS.medium },
  pickerRow: { flexDirection: 'row' },
  pickerBox: { flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, paddingHorizontal: 8, paddingVertical: 5, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  pickerLabel: { fontSize: 8, fontWeight: '900', color: 'rgba(255,255,255,0.8)', marginLeft: 5 },
  ...
  paperSchoolInfo: { flex: 1 },
  paperSchoolName: { fontSize: 16, fontWeight: '900', color: '#1E293B', textTransform: 'uppercase' },
  paperContact: { fontSize: 10, color: '#64748B', marginTop: 2 },
  divider: { height: 2, backgroundColor: '#1E293B', marginVertical: 10 },
  receiptTitle: { textAlign: 'center', fontSize: 14, fontWeight: '900', color: '#1E293B', letterSpacing: 2, marginBottom: 20 },
  paperInfoGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 25 },
  infoItem: { width: '48%' },
  ...
  tableHeader: { flexDirection: 'row', borderBottomWidth: 1, borderTopWidth: 1, borderColor: '#000', paddingVertical: 8 },
  ...
  totalsRow: { flexDirection: 'row', justifyContent: 'flex-end', gap: 20, marginBottom: 6 },
  totalsLabel: { fontSize: 10, fontWeight: '800', color: '#64748B' },
  totalsValue: { fontSize: 11, fontWeight: '700', color: '#1E293B', width: 100, textAlign: 'right' },
  grandTotalRow: { flexDirection: 'row', justifyContent: 'flex-end', gap: 20, marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderStyle: 'dashed' },
  grandTotalLabel: { fontSize: 12, fontWeight: '900', color: '#1E293B' },
  grandTotalValue: { fontSize: 14, fontWeight: '900', color: COLORS.primary, width: 100, textAlign: 'right' },
  paperFooter: { marginTop: 50, alignItems: 'center' },
  footerText: { fontSize: 9, color: '#94A3B8', fontStyle: 'italic', textAlign: 'center' },
  copyrightText: { fontSize: 9, fontWeight: '800', color: '#CBD5E1', marginTop: 4 },
