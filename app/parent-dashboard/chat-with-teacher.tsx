...
export default function ParentChatWithTeacher() {
  ...
  const sendVoiceMessage = async () => {
    if ((!previewUri && !webBlobRef.current) || !chatId) return;
    try {
      setUploading(true);
      let blob: Blob | null = null;
      if (Platform.OS === "web") blob = webBlobRef.current;
      else if (previewUri) blob = await (await fetch(previewUri)).blob();
      if (!blob) return;
      const ext = Platform.OS === "web" ? "webm" : "m4a";
      const audioRef = ref(storage, `chats/${chatId}/${Date.now()}.${ext}`);
