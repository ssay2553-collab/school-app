...
export default function StudentFeeHistory() {
  ...
  const [selectedChildId, setSelectedChildId] = useState<string>((paramStudentId as string) || "");
  ...
  useEffect(() => {
    ...
    const fetchChildren = async () => {
      ...
      try {
        const q = query(collection(db, "users"), where(documentId(), "in", ids.slice(0, 30)));
      ...
      <LinearGradient colors={[primary, secondary]} style={styles.header}>
        <View style={styles.headerTop}>
            ...
            <TouchableOpacity onPress={generatePDF} disabled={!record}>
                <SVGIcon name="cloud-download" size={24} color={!record ? "rgba(255,255,255,0.3)" : "#fff"} />
            </TouchableOpacity>
        </View>
        <View style={styles.filterRow}>
          <View style={[styles.pickerBox, { flex: 1.2 }]}>
            ...
            <Picker selectedValue={selectedYear} onValueChange={setSelectedYear} style={[styles.picker, Platform.OS === 'web' && { color: '#fff', backgroundColor: 'transparent', outline: 'none', border: 'none' } as any]} dropdownIconColor="#fff">
              {availableYears.map(y => <Picker.Item key={y} label={y} value={y} color={Platform.OS === 'web' ? '#000' : '#0F172A'} />)}
            </Picker>
          </View>
          <View style={[styles.pickerBox, { flex: 1, marginLeft: 10 }]}>
            <Text style={styles.miniLabel}>TERM {selectedTerm === acadConfig.currentTerm ? "(CURRENT)" : ""}</Text>
            <Picker selectedValue={selectedTerm} onValueChange={setSelectedTerm} style={[styles.picker, Platform.OS === 'web' && { color: '#fff', backgroundColor: 'transparent', outline: 'none', border: 'none' } as any]} dropdownIconColor="#fff">
              <Picker.Item label="Term 1" value="Term 1" color={Platform.OS === 'web' ? '#000' : '#0F172A'} />
              <Picker.Item label="Term 2" value="Term 2" color={Platform.OS === 'web' ? '#000' : '#0F172A'} />
              <Picker.Item label="Term 3" value="Term 3" color={Platform.OS === 'web' ? '#000' : '#0F172A'} />
            </Picker>
          </View>
        </View>
      </LinearGradient>
        ...
        {children.length > 1 && (
            <View style={styles.selectorWrapper}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.selectorScroll}>
                    {children.map(c => (
                        <TouchableOpacity key={c.id} onPress={() => setSelectedChildId(c.id)} style={[styles.childChip, selectedChildId === c.id && { backgroundColor: primary, borderColor: primary }]}>
                            <Text style={[styles.childChipText, selectedChildId === c.id && { color: '#fff' }]}>{c.name}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
            </View>
        ...
        {fetchingRecord ? (
            ...
            <Animatable.View animation="fadeInUp" duration={600} style={styles.receiptPaper}>
                ...
                <View style={styles.paperHeader}>
                    ...
                    <View style={{ flex: 1 }}>
                        <Text style={[styles.receiptSchoolName, { color: primary }]}>{SCHOOL_CONFIG.fullName}</Text>
                ...
                <View style={styles.paperInfoGrid}>
                    ...
                    <View style={styles.paperInfoItem}>
                        ...
                        <Text style={styles.paperInfoValue}>{record.className || "N/A"}</Text>
                    </View>
                    <View style={styles.paperInfoItem}>
                        <Text style={styles.paperInfoLabel}>PERIOD:</Text>
                        <Text style={styles.paperInfoValue}>{record.term} • {record.academicYear}</Text>
                    </View>
                    <View style={styles.paperInfoItem}>
                        <Text style={styles.paperInfoLabel}>DATE:</Text>
                        <Text style={styles.paperInfoValue}>{moment().format("DD/MM/YYYY")}</Text>
                    </View>
                </View>
                ...
                <View style={styles.tableContainer}>
                    ...
                    {(record.payments || []).length === 0 ? (
                        ...
                        <View key={i} style={styles.tableRow}>
                            <Text style={[styles.td, { flex: 1 }]}>{moment(p.createdAt).format("DD/MM/YY")}</Text>
                            <Text style={[styles.td, { flex: 1.5 }]} numberOfLines={1}>{p.receiptNo || 'RC-' + String(p.createdAt).slice(-6)}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: 'right' }]}>₵{Number(p.amount).toFixed(2)}</Text>
                        </View>
                    ))}
                </View>
                ...
                <View style={styles.totalsSection}>
                    <View style={styles.totalsRow}>
                        ...
                        <Text style={styles.totalsValue}>₵ {record.arrears?.toFixed(2)}</Text>
                    </View>
                    <View style={styles.totalsRow}>
                        <Text style={styles.totalsLabel}>TERM BILL:</Text>
                        <Text style={styles.totalsValue}>₵ {record.termBill?.toFixed(2)}</Text>
                    </View>
                    <View style={styles.totalsRow}>
                        <Text style={styles.totalsLabel}>TOTAL PAID:</Text>
                        <Text style={[styles.totalsValue, { color: '#10B981' }]}>- ₵ {record.amountPaid?.toFixed(2)}</Text>
                    </View>
                    <View style={styles.grandTotalRow}>
                        <Text style={styles.grandTotalLabel}>BALANCE DUE:</Text>
                        <Text style={[styles.grandTotalValue, { color: primary }]}>₵ {record.balance?.toFixed(2)}</Text>
                    </View>
                </View>

                <View style={styles.paperFooter}>
                    <Text style={styles.footerNote}>Computer generated. No signature required.</Text>
                    <Text style={styles.footerCopyright}>© {moment().year()} {SCHOOL_CONFIG.fullName}</Text>
                </View>
                ...
                <TouchableOpacity onPress={generatePDF} style={[styles.printBtn, { backgroundColor: primary }]}>
                    <SVGIcon name="download" size={24} color="#fff" />
                    <Text style={styles.printBtnText}>Download Official PDF</Text>
                </TouchableOpacity>
            </Animatable.View>
        ) : selectedChildId && <View style={styles.emptyContainer}><SVGIcon name="alert-circle" size={64} color="#CBD5E1" /><Text style={styles.emptyTitle}>No record found</Text><Text style={styles.emptySub}>No financial data exists for this student in {selectedTerm} {selectedYear}.</Text></View>}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  header: { paddingBottom: 25, ...SHADOWS.medium, borderBottomLeftRadius: 35, borderBottomRightRadius: 35 },
  headerTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 25, paddingTop: Platform.OS === 'ios' ? 10 : 40, marginBottom: 20 },
  backBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center' },
  headerTitle: { fontSize: 18, fontWeight: '900', color: '#fff' },
  headerSub: { fontSize: 11, color: 'rgba(255,255,255,0.7)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 },
  logoMini: { width: 32, height: 32 },
  filterRow: { flexDirection: 'row', paddingHorizontal: 25, gap: 10 },
  pickerBox: { flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 14, height: 60, justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)', paddingHorizontal: 5 },
  picker: { height: 40, color: '#fff', marginLeft: -10, marginTop: 15 },
  miniLabel: { fontSize: 9, fontWeight: '900', color: 'rgba(255,255,255,0.8)', position: 'absolute', top: 8, left: 14, zIndex: 1, letterSpacing: 0.5 },
  scrollContent: { padding: 20 },
  selectorWrapper: { marginBottom: 20 },
  sectionLabel: { fontSize: 10, fontWeight: '900', color: '#94A3B8', marginBottom: 12, letterSpacing: 1 },
  selectorScroll: { gap: 10 },
  childChip: { paddingHorizontal: 18, paddingVertical: 10, borderRadius: 15, backgroundColor: '#fff', borderWidth: 1, borderColor: '#E2E8F0', ...SHADOWS.small },
  ...
  receiptSchoolName: { fontSize: 16, fontWeight: '900', textTransform: 'uppercase' },
  receiptMotto: { fontSize: 10, fontStyle: 'italic', color: '#64748B' },
  paperDivider: { height: 2, backgroundColor: '#1E293B', marginVertical: 10 },
  receiptTitleText: { textAlign: 'center', fontSize: 14, fontWeight: '900', color: '#1E293B', letterSpacing: 2, marginBottom: 20 },
  ...
  noPayments: { textAlign: 'center', padding: 20, color: '#94A3B8', fontSize: 12, fontStyle: 'italic' },
  ...
  grandTotalValue: { fontSize: 14, fontWeight: '900', width: 100, textAlign: 'right' },

  paperFooter: { marginTop: 50, alignItems: 'center' },
  footerNote: { fontSize: 9, color: '#94A3B8', fontStyle: 'italic' },
  footerCopyright: { fontSize: 9, fontWeight: '800', color: '#CBD5E1', marginTop: 4 },
