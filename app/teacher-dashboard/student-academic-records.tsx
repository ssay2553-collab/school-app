...
export default function StudentAcademicRecords() {
  ...
  const [signatureUrl, setSignatureUrl] = useState(appUser?.profile?.signatureUrl || "");
  ...
  const calculateScores = useCallback((student: StudentScoreRecord, type: ReportType) => {
    ...
    if (type === "End of Term") {
      ...
      const finalScoreNum = parseFloat(updated.classScore) + parseFloat(updated.exam50);
  ...
  useEffect(() => {
    ...
    const fetchTeacherMetadata = async () => {
      ...
      try {
        ...
        setSignatureUrl(appUser.profile?.signatureUrl || "");
      } catch (err) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const list: StudentScoreRecord[] = snap.docs.map((d) => ({
            ...
            catA: "", catB: "", groupWork: "", projectWork: "", total60: 0, classScore: "0",
            examsMark: "", exam50: "0", finalScore: "0", grade: "N/A",
            conduct: "Excellent", interest: "High", attitude: "Positive", teacherRemarks: "",
          }));
          setAllStudents(list);
  ...
  const updateStudentScore = (studentId: string, field: keyof StudentScoreRecord, value: string) => {
  ...
  const saveRecord = async () => {
    if (!selectedClassId || !selectedSubject || !term || !selectedYear) return Alert.alert("Error", "Missing fields.");
    try {
      const yearSlug = selectedYear.replace(/\//g, "-");
      const reportSlug = reportType.replace(/\s+/g, "");
      const docId = `${selectedClassId}_${selectedSubject.replace(/\s+/g, "")}_${yearSlug}_${term.replace(/\s+/g, "")}_${reportSlug}`;
      await setDoc(doc(db, "academicRecords", docId), {
        docId, teacherId: appUser?.uid, classId: selectedClassId,
        className: teacherClasses.find((c) => c.id === selectedClassId)?.name || selectedClassId,
        subject: selectedSubject, academicYear: selectedYear, term, reportType,
        students: allStudents, timestamp: serverTimestamp(), status: "pending",
  ...
  const handleUploadSignature = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0].uri) {
        ...
        await updateDoc(doc(db, "users", appUser!.uid), { "profile.signatureUrl": downloadURL });
  ...
  if (loading || acadConfig.loading) return <View style={styles.center}><ActivityIndicator size="large" color={COLORS.primary} /></View>;
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        {isClassTeacher && (
          <Animatable.View animation="fadeInDown" style={styles.signatureCard}>
            ...
            <Text style={styles.sigSubtitle}>Required for official terminal reports</Text>
            <View style={styles.sigContent}>
              {signatureUrl ? (
                <Image source={{ uri: signatureUrl }} style={styles.sigImage} resizeMode="contain" />
              ) : (
              ...
              <TouchableOpacity style={[styles.sigUploadBtn, { backgroundColor: COLORS.primary }]} onPress={handleUploadSignature} disabled={uploadingSig}>
                {uploadingSig ? <ActivityIndicator color="#fff" /> : <Text style={styles.sigUploadBtnText}>{signatureUrl ? "Replace Signature" : "Upload Signature"}</Text>}
              </TouchableOpacity>
            </View>
          </Animatable.View>
        )}
        ...
        <Animatable.View animation="fadeInDown" style={styles.configCard}>
          ...
          <Text style={styles.label}>ACADEMIC YEAR {selectedYear === acadConfig.academicYear && "(CURRENT)"}</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bubbleRow}>
            {academicYears.map((y) => (
              <TouchableOpacity key={y} onPress={() => setSelectedYear(y)} style={[styles.bubble, selectedYear === y && styles.bubbleActive]}>
                <Text style={[styles.bubbleText, selectedYear === y && styles.bubbleTextActive]}>{y}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <Text style={styles.label}>TERM {term === acadConfig.currentTerm && "(CURRENT)"}</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bubbleRow}>
            {["Term 1", "Term 2", "Term 3"].map((t) => (
              <TouchableOpacity key={t} onPress={() => setTerm(t)} style={[styles.bubble, term === t && styles.bubbleActive]}>
                <Text style={[styles.bubbleText, term === t && styles.bubbleTextActive]}>{t}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <Text style={styles.label}>REPORT TYPE</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bubbleRow}>
            {(["End of Term", "Mid-Term", "Mock Exams"] as ReportType[]).map((type) => (
              <TouchableOpacity key={type} onPress={() => setReportType(type)} style={[styles.bubble, reportType === type && { backgroundColor: COLORS.secondary, borderColor: COLORS.secondary }]}>
                <Text style={[styles.bubbleText, reportType === type && styles.bubbleTextActive]}>{type}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bubbleRow}>
            {teacherClasses.map((c) => (
              <TouchableOpacity key={c.id} onPress={() => setSelectedClassId(c.id)} style={[styles.bubble, selectedClassId === c.id && styles.bubbleActive]}>
                <Text style={[styles.bubbleText, selectedClassId === c.id && styles.bubbleTextActive]}>{c.name}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        ...
        {syncing ? (
          ...
          <View style={styles.recordsList}>
            <View style={styles.listHeader}><Text style={styles.listTitle}>STUDENT PERFORMANCE LIST</Text><Text style={styles.listCount}>{allStudents.length} Students</Text></View>
            {visibleStudents.map((student) => (
              <Animatable.View key={student.studentId} animation="fadeInUp" style={styles.studentCard}>
                <View style={styles.cardHeader}><Text style={styles.studentName}>{student.fullName}</Text><View style={styles.gradeBadge}><Text style={styles.gradeText}>{student.grade}</Text></View></View>
                <View style={styles.scoresGrid}>
                  {reportType === "End of Term" ? (
                    <>
                      <View style={styles.scoreInput}><Text style={styles.scoreLabel}>CAT A (15)</Text><TextInput value={student.catA} onChangeText={(v) => updateStudentScore(student.studentId, "catA", v)} keyboardType="numeric" style={styles.input} /></View>
                      <View style={styles.scoreInput}><Text style={styles.scoreLabel}>CAT B (15)</Text><TextInput value={student.catB} onChangeText={(v) => updateStudentScore(student.studentId, "catB", v)} keyboardType="numeric" style={styles.input} /></View>
                      <View style={styles.scoreInput}><Text style={styles.scoreLabel}>GROUP (15)</Text><TextInput value={student.groupWork} onChangeText={(v) => updateStudentScore(student.studentId, "groupWork", v)} keyboardType="numeric" style={styles.input} /></View>
                      <View style={styles.scoreInput}><Text style={styles.scoreLabel}>PROJECT (15)</Text><TextInput value={student.projectWork} onChangeText={(v) => updateStudentScore(student.studentId, "projectWork", v)} keyboardType="numeric" style={styles.input} /></View>
                      <View style={styles.scoreInput}><Text style={styles.scoreLabel}>EXAMS (100)</Text><TextInput value={student.examsMark} onChangeText={(v) => updateStudentScore(student.studentId, "examsMark", v)} keyboardType="numeric" style={styles.input} /></View>
                      <View style={styles.totalBox}><Text style={styles.totalLabel}>TOTAL (100)</Text><Text style={styles.totalVal}>{student.finalScore}</Text></View>
                    </>
                  ) : (
                    <View style={[styles.scoreInput, { flex: 1 }]}><Text style={styles.scoreLabel}>TOTAL SCORE</Text><TextInput value={student.examsMark} onChangeText={(v) => updateStudentScore(student.studentId, "examsMark", v)} keyboardType="numeric" style={styles.input} /></View>
                  )}
                </View>
                {isClassTeacher && reportType === "End of Term" && (
                  <View style={styles.behavioralGrid}>
                    <View style={styles.scoreInput}><Text style={styles.scoreLabel}>CONDUCT</Text><TextInput value={student.conduct} onChangeText={(v) => updateStudentScore(student.studentId, "conduct", v)} style={styles.input} /></View>
                    <View style={styles.scoreInput}><Text style={styles.scoreLabel}>INTEREST</Text><TextInput value={student.interest} onChangeText={(v) => updateStudentScore(student.studentId, "interest", v)} style={styles.input} /></View>
                    <View style={styles.scoreInput}><Text style={styles.scoreLabel}>ATTITUDE</Text><TextInput value={student.attitude} onChangeText={(v) => updateStudentScore(student.studentId, "attitude", v)} style={styles.input} /></View>
                    <View style={[styles.scoreInput, { width: '100%' }]}><Text style={styles.scoreLabel}>TEACHER REMARKS</Text><TextInput value={student.teacherRemarks} onChangeText={(v) => updateStudentScore(student.studentId, "teacherRemarks", v)} style={styles.input} multiline /></View>
                  </View>
                )}
              </Animatable.View>
            ))}
            {allStudents.length > visibleStudents.length && (
              <TouchableOpacity onPress={loadMore} style={styles.loadMoreBtn}><Text style={styles.loadMoreText}>LOAD MORE STUDENTS</Text></TouchableOpacity>
            )}
          </View>
        )}
      </ScrollView>

      <TouchableOpacity onPress={saveRecord} style={styles.saveFab}><LinearGradient colors={[COLORS.primary, "#4F46E5"]} style={styles.fabGrad}><Text style={styles.saveFabText}>SAVE PERFORMANCE LEDGER</Text><Ionicons name="checkmark-done" size={24} color="#fff" /></LinearGradient></TouchableOpacity>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  signatureCard: { backgroundColor: "#fff", margin: 20, padding: 20, borderRadius: 24, ...SHADOWS.small, borderWidth: 1, borderColor: "#E2E8F0" },
  sigHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 4 },
  sigTitle: { fontSize: 16, fontWeight: "800", color: "#1E293B" },
  sigSubtitle: { fontSize: 12, color: "#64748B", marginBottom: 15, fontWeight: "600" },
  sigContent: { alignItems: "center" },
  sigImage: { width: "100%", height: 100, backgroundColor: "#F8FAFC", borderRadius: 12, borderWidth: 1, borderColor: "#E2E8F0" },
  sigPlaceholder: { width: "100%", height: 100, backgroundColor: "#F1F5F9", borderRadius: 12, borderStyle: "dashed", borderWidth: 2, borderColor: "#CBD5E1", justifyContent: "center", alignItems: "center" },
  sigPlaceholderText: { fontSize: 12, color: "#94A3B8", marginTop: 8, fontWeight: "700" },
  sigUploadBtn: { marginTop: 15, paddingVertical: 10, paddingHorizontal: 20, borderRadius: 12 },
  ...
  bubble: { paddingVertical: 10, paddingHorizontal: 18, borderRadius: 15, backgroundColor: "#F1F5F9", borderWidth: 1, borderColor: "#E2E8F0" },
  ...
  behavioralGrid: { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderColor: "#F1F5F9", flexDirection: "row", flexWrap: "wrap", gap: 10 },
