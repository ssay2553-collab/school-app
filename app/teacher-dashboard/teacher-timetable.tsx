...
const LessonCard = React.memo(({ lesson, brandColor, type = 'lesson' }: { lesson: Lesson, brandColor: string, type?: 'lesson' | 'other' }) => {
...
export default function TeacherTimetable() {
  ...
  const fetchTimetablesAndNames = useCallback(async () => {
    ...
    try {
      ...
      for (const chunk of classChunks) {
        const classesQuery = query(collection(db, "classes"), where("__name__", "in", chunk));
        const classSnaps = await getDocs(classesQuery);
        classSnaps.forEach(doc => {
          nameResult[doc.id] = doc.data().name || doc.id;
        });

        const ttQuery = query(collection(db, "timetables"), where("__name__", "in", chunk));
  ...
  const sortedClassIds = useMemo(() => {
    const list = Object.keys(classNames).map(id => ({ id, name: classNames[id] }));
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ marginTop: 10, color: '#64748B' }}>Optimizing schedule...</Text>
      </View>
    );
  }
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.daySelector}>
          {DAYS.map((day) => (
            <TouchableOpacity
              ...
              style={[
                styles.dayTab,
                selectedDay === day && { backgroundColor: '#fff', borderColor: '#fff' }
              ]}
            >
              <Text style={[styles.dayTabText, selectedDay === day && { color: brandColor }]}>
      ...
      >
        {isClassTeacher && (
             <TouchableOpacity 
                ...
                onPress={() => router.push("/admin-dashboard/CreateLessonTimetable")}
             >
                <SVGIcon name="create" size={20} color={brandColor} />
                <Text style={[styles.manageBtnText, { color: brandColor }]}>Manage Class Timetable</Text>
             </TouchableOpacity>
        )}
        ...
        {sortedClassIds.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>No classes assigned to your portal.</Text>
          </View>
        ) : (
          sortedClassIds.map((classId, index) => {
            const lessons = timetables[classId]?.timetableDays?.[selectedDay] || [];
            const otherActs = timetables[classId]?.otherActivities || [];
            const isAssignedClass = classId === appUser?.classTeacherOf;
            
            if (lessons.length === 0 && otherActs.length === 0 && !isAssignedClass) return null;

            return (
              <Animatable.View animation="fadeInUp" delay={index * 50} key={classId} style={styles.classSection}>
                <View style={styles.classHeader}>
                  <View style={[styles.classIcon, { backgroundColor: brandColor + '10' }]}>
                    <SVGIcon name="people" size={18} color={brandColor} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.classTitle}>{classNames[classId]}</Text>
                    {isAssignedClass && <Text style={styles.myClassTag}>My Assigned Class</Text>}
                  </View>
                </View>

                {lessons.map((lesson, idx) => (
                  <LessonCard key={`lesson-${idx}`} lesson={lesson} brandColor={brandColor} />
                ))}

                {otherActs.map((act, idx) => (
                  <LessonCard key={`act-${idx}`} lesson={act} brandColor={brandColor} type="other" />
                ))}

                {lessons.length === 0 && otherActs.length === 0 && isAssignedClass && (
                    <View style={styles.emptyDayBox}>
                        <Text style={styles.emptyDayText}>No schedule for this day yet.</Text>
                    </View>
                )}
              </Animatable.View>
            );
...
const styles = StyleSheet.create({
  ...
  headerSub: { fontSize: 11, color: "rgba(255,255,255,0.7)", fontWeight: "700", marginTop: -2, textTransform: 'uppercase' },
  ...
  dayTabText: { fontSize: 12, fontWeight: "900", color: "rgba(255,255,255,0.6)" },
  manageBtn: { 
    flexDirection: 'row', 
    alignItems: 'center', 
    justifyContent: 'center', 
  ...
  classHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 15, gap: 12 },
  classIcon: { width: 36, height: 36, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  classTitle: { fontSize: 18, fontWeight: "800", color: "#1E293B", textTransform: 'uppercase' },
  myClassTag: { fontSize: 10, color: '#64748B', fontWeight: '800', textTransform: 'uppercase', marginTop: 2 },
  lessonCard: {
    flexDirection: "row",
    alignItems: "center",
    padding: 16,
  ...
  emptyDayBox: { padding: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', borderRadius: 15, borderStyle: 'dashed', borderWidth: 1, borderColor: '#E2E8F0' },
