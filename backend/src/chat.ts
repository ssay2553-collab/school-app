 ...
 */
export const onNewChatMessage = onDocumentCreated("directMessages/{chatId}/messages/{messageId}", async (event) => {
  ...
  try {
    ...
    const message = {
      notification: {
        ...
        body: text ? (text.length > 100 ? text.substring(0, 97) + "..." : text) : "Sent an attachment",
      },
 ...
 */
export const onNewGroupMessage = onDocumentCreated("studentGroups/{groupId}/messages/{messageId}", async (event) => {
  const messageData = event.data?.data();
  if (!messageData) return;
  ...
  try {
    ...
    const groupDoc = await admin.firestore().doc(`studentGroups/${groupId}`).get();
    const groupData = groupDoc.data();
    if (!groupData) return;

    const groupName = groupData.name || "Group Chat";
    const participants: string[] = [groupData.teacherId, ...(groupData.studentIds || [])];
    ...
    console.log(`Sent ${response.successCount} group notifications for ${groupId}`);
 ...
 */
export const onNewGuestMessage = onDocumentCreated("guestTickets/{ticketId}/messages/{messageId}", async (event) => {
  const messageData = event.data?.data();
  if (!messageData) return;
  ...
  try {
    const ticketDoc = await admin.firestore().doc(`guestTickets/${ticketId}`).get();
...
export const cleanupTask = onSchedule("0 * * * *", async () => {
  ...
  try {
    ...
    for (const chatDoc of directChatsSnap.docs) {
      ...
      const oldMessagesSnap = await messagesRef.where("createdAt", "<", cutoffMessages).get();
  ...
  } catch (e) { console.error("Old message cleanup error:", e); }
